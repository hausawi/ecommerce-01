	...
	return (
		<div className='flex items-center justify-between py-5 font-medium'>
			...
			<ul className='hidden sm:flex gap-5 text-sm text-gray-700'>
				...
				<NavLink className='flex flex-col items-center gap-1' to={'/platform'}>
					<p>المنصة</p>
					<hr className='w-2/4 border-none h-[1.5px] bg-[#0046BB] hidden' />
				</NavLink>
			</ul>
			...
			<div className='flex items-center gap-6'>
					...
					{token && (
						<div className='group-hover:block hidden absolute dropdown-menu right-0 pt-4'>
							<div className='flex flex-col gap-2 w-36 py-3 px-5 bg-[#5BAEB7] text-gray-700'>
								<p className='cursor-pointer hover:text-black'>الحساب </p>
								<p
				...
				<Link to='/cart' className='relative'>
					...
					<p className='absolute right-[-5px] top-[-5px] w-4 text-center leading-4 bg-[#0046BB] text-white aspect-square rounded-full text-[8px]'>
						{getCartCount()}
					</p>
				</Link>
				<img
			...
			<div
				...
				<div className='flex flex-col text-gray-600 '>
					...
					<NavLink
						onClick={() => setVisible(false)}
						className='py-2 pl-6 border'
						to='https://sudanesedream.org/login'>
						المنصة
					</NavLink>
				</div>
			</div>
